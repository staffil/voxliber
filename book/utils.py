...
def export_voice_previews_to_local():
    """
    VoiceList에 저장된 sample_audio 파일들을
    voice_name + voice_id 기준 파일명으로
    F:\\voicePreview 디렉토리에 저장
    """

    TARGET_DIR = r"F:\voicePreview"
    os.makedirs(TARGET_DIR, exist_ok=True)

    voices = VoiceList.objects.exclude(sample_audio__isnull=True)

    print(f"🎧 총 {voices.count()}개 음성 처리 시작")

    for voice in voices:
        try:
            if not voice.sample_audio:
                continue

            # 원본 파일 경로 (MEDIA_ROOT 기준)
            source_path = voice.sample_audio.path

            # 안전한 파일명
            safe_name = voice.voice_name.replace(" ", "_")
            filename = f"{safe_name}_{voice.voice_id}.mp3"
            ...
            print(f"⚠️ 실패: {voice.voice_name} ({voice.voice_id}) - {e}")

    print("🎉 모든 preview 파일 저장 완료")
